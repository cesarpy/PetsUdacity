...
public class EditorActivity extends AppCompatActivity implements LoaderManager.LoaderCallbacks<Cursor> {
            ...
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                ...
                if (!TextUtils.isEmpty(selection)) {
                    if (selection.equals(getString(R.string.gender_male))) {
                        mGender = PestsContract.PetEntry.GENDER_MALE; // Male
                    } else if (selection.equals(getString(R.string.gender_female))) {
                        mGender = PestsContract.PetEntry.GENDER_FEMALE; // Female
                    } else {
                        mGender = PestsContract.PetEntry.GENDER_UNKNOWN; // Unknown
                    }
                }
            }
    ...
    private void insertPet(){
        ...
        if (mUri == null &&
                TextUtils.isEmpty(mNameEditText.getText()) && TextUtils.isEmpty(mBreedEditText.getText()) &&
        ...
        Integer up = null;
        if(onUpdate){
            up = getContentResolver().update(mUri,values, null, null);
        }else{
            // Insert a new pet into the provider, returning the content URI for the new pet.
             newUri = getContentResolver().insert(PestsContract.PetEntry.CONTENT_URI, values);
        }

        // Show a toast message depending on whether or not the insertion was successful
        if (newUri == null && up == null) {
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        switch (item.getItemId()) {
            ...
            case android.R.id.home:
                //este es para la flechita de atras
...
// Bail early if the cursor is null or there is less than 1 row in the cursor
        ...
        if (cursor.moveToFirst()) {
            ...
            switch (gender) {
                case PestsContract.PetEntry.GENDER_MALE:
